...
def sqlite_connect():
    """
        ...
        :return sqlite_con: The active connection
    """
    try:
...
def create_table(sqlite_con):
    ...
    py_cursor.execute('CREATE TABLE IF NOT EXISTS resources(jobID integer PRIMARY KEY, memory integer, cores integer, '
                      'time integer)')
    sqlite_con.commit()
...
def insert_data(sqlite_con, ph):
    ...
    """
    py_cursor = sqlite_con.cursor()
    py_cursor.execute('INSERT OR IGNORE INTO resources(jobID, memory, cores, time) VALUES(?, ?, ?, ?)', ph)
    sqlite_con.commit()
